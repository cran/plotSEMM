`plotSEMM_contour` <-
function(EtaN2=Eta2, EtaN1=Eta1, classinfo=TRUE, lnty = 3, lncol = 1, title = '', leg = TRUE) {
...
if(classinfo == TRUE) {for (i in 1:SEMLIdatapks$classes[1]) {
lines(SEMLIdatapks$Ksi,SEMLIdatapks$pKsi[,i],lwd=1,lty=(i+lnty),col=(i+lncol)) } }

#legend
if (leg == TRUE) {classes <- SEMLIdatapks$classes[1]
...
legend(x="topright", legend=text, horiz=FALSE,
    lwd=lwd1,lty=lty1,col=col1,, bty="n") }

# plot3 contour
par(mar=c(4,4,0,0))
plot(SEMLIdatapks$Ksi,SEMLIdatapks$Eta,type='n',xlab=xlabel,
ylab=ylabel,main='')
